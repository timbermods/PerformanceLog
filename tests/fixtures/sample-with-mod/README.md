...
This folder is one recording of one Timberborn session, made by the **Performance Log** mod (version 0.1.0). It holds measurements
...
and totals for allocation, counts and the two histograms. The frame-time histogram's bucket edges in milliseconds are 4, 6, 8.5, 11.5, 14, 17.5, 21, 25, 30, 35, 42, 50, 75, 100, 200, 400 (`fh0`..`fh16`;
